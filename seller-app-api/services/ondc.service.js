...
class OndcService {
  async productSearch(payload = {}, req = {}) {
    try {
      ...
      logger.log(
        "info",
        `[Ondc Service] search logistics payload : param >>:`,
        payload
      );
      ...
      logger.error(
        "error",
        `[Ondc Service] search logistics payload - search logistics payload : param :`,
        err
      );
      throw err;
    }
  ...
  async orderSelect(payload = {}, req = {}) {
    try {
      ...
      let org = await productService.getOrgForOndc(
        payload.message.order.provider.id
      );
  ...
  async postSelectRequest(searchRequest, logisticsMessageId, selectMessageId) {
    try {
      ...
      setTimeout(() => {
        logger.log(
          "info",
          `[Ondc Service] search logistics payload - timeout : param :`,
          searchRequest
        );
  ...
  async buildSelectRequest(logisticsMessageId, selectMessageId) {
    try {
      logger.log(
        "info",
        `[Ondc Service] search logistics payload - build select request : param :`,
        { logisticsMessageId, selectMessageId }
      );
      //1. look up for logistics
      let logisticsResponse = await this.getLogistics(
        logisticsMessageId,
        selectMessageId,
        "select"
      );
      ...
      let selectResponse = await productService.productSelect(
        logisticsResponse
      );
  ...
  async buildSearchRequest(searchRequest, searchMessageId) {
    try {
      let searchResponse = await productService.search(
        searchRequest,
        searchMessageId
      );
  ...
  async getLogistics(logisticsMessageId, retailMessageId, type) {
    try {
      logger.log("info", `[Ondc Service] get logistics : param :`, {
        logisticsMessageId,
        retailMessageId,
        type,
      });
      ...
      console.log(
        `🚀 ~ file: ondc.service.js:286 ~ OndcService ~ getLogistics ~  config.get("sellerConfig").BPP_URI:`,
        config.get("sellerConfig").BPP_URI
      );
  ...
  async postSelectResponse(selectResponse) {
    try {
      logger.info(
        "info",
        `[Ondc Service] post http select response : `,
        selectResponse
      );
  ...
  async orderInit(payload = {}, req = {}) {
    try {
      ...
      logger.log(
        "info",
        `[Ondc Service] init logistics payload : param :`,
        ...
        storeLocationEnd = {
          location: {
            ...
            address: {
              ...
              locality:
                org.providerDetail.storeDetails.address.locality || "delhi",
      ...
      let deliveryType = logistics.message.catalog[
        "bpp/providers"
      ][0].items.find((element) => {
        return element.category_id === "Standard Delivery";
      });
      ...
      const initRequest = {
        ...
        message: {
          order: {
            ...
            billing: {
              //TODO: discuss whos details should go here buyer or seller
      ...
      logger.error("error", `[Ondc Service] build init request :`, {
        error: err.stack,
        message: err.message,
      });
      console.log(err);
  ...
  async buildInitRequest(logisticsMessageId, initMessageId) {
    try {
      logger.log("info", `[Ondc Service] build init request :`, {
        logisticsMessageId,
        initMessageId,
      });
      ...
      logger.log(
        "info",
        `[Ondc Service] build init request - get logistics response :`,
        logisticsResponse
      );
  ...
  async postInitResponse(initResponse) {
    try {
      ...
      logger.error("error", `[Ondc Service] post init request :`, {
        error: err.stack,
        message: err.message,
      });
      return err;
  ...
  async orderConfirm(payload = {}, req = {}) {
    try {
        ...
        storeLocationEnd = {
          location: {
            ...
            address: {
              ...
              name: org.providerDetail.name,
              building: org.providerDetail.storeDetails.address.building,
              locality: org.providerDetail.storeDetails.address.locality || "mohali",
              city: org.providerDetail.storeDetails.address.city,
              state: org.providerDetail.storeDetails.address.state,
              country: org.providerDetail.storeDetails.address.country,
            },
      ...
      let end = { ...order.fulfillments[0].end };
        ...
        end.location.address.locality ?? end.location.address.street;
      ...
      let deliveryType = selectRequest.selectedLogistics.message.catalog[
        "bpp/providers"
      ][0].items.find((element) => {
        return element.category_id === "Standard Delivery";
      });
      ...
      logger.info(
        "info",
        `[Ondc Service] post init request :confirmRequestconfirmRequestconfirmRequestconfirmRequestconfirmRequestconfirmRequest`,
        confirmRequest
      );
      this.postConfirmRequest(
        confirmRequest,
        logisticsMessageId,
        selectMessageId
      );
  ...
  async buildConfirmRequest(logisticsMessageId, initMessageId) {
    try {
      ...
      let selectResponse = await productService.productConfirm(
        logisticsResponse
      );
  ...
  async orderStatusUpdate(payload = {}, req = {}) {
    try {
      ...
      // setTimeout(this.getLogistics(logisticsMessageId,selectMessageId),3000)
      //setTimeout(() => {
      this.postUpdateOrderStatusRequest(
        payload,
        trackRequest,
        logisticsMessageId,
        selectMessageId
        ...
        message: {
          order: {
            ...
            "@ondc/org/linked_order": {
              items: [
                {
                  //TODO: get valid item from list and update the fields
                  descriptor: {
                    name: "KIT KAT",
                  },
                  quantity: {
  ...
  async orderStatusUpdateItems(payload = {}, req = {}) {
    try {
      ...
      payload = {
        message: { order: order },
        context: {
          ...confirmRequest.confirmRequest.context,
          message_id: uuidv4(),
        },
      };
      // setTimeout(this.getLogistics(logisticsMessageId,selectMessageId),3000)
      //setTimeout(() => {
      this.postUpdateItemRequest(
        payload,
        trackRequest,
        logisticsMessageId,
        selectMessageId
  ...
  async postUpdateOrderStatusRequest(
    orderData,
    searchRequest,
    logisticsMessageId,
    selectMessageId
    ...
    try {
      ...
      try {
        //TODO: post this request for update items
  ...
  async buildStatusRequest(
    ...
    try {
      ...
      let statusResponse = await productService.productStatus(
        logisticsResponse,
        statusRequest,
        unsoliciated,
        payload
  ...
  async buildUpdateRequest(statusRequest, logisticsMessageId, initMessageId) {
    try {
      ...
      let statusResponse = await productService.productUpdate(
        logisticsResponse
      );
  ...
  async buildUpdateItemRequest(
    ...
    try {
      ...
      let statusResponse = await productService.productUpdateItem(
        statusRequest,
        logisticsResponse
      );
  ...
  async buildOrderStatusRequest(
    ...
    try {
      ...
      let statusResponse = await productService.productOrderStatus(
        logisticsResponse,
        statusRequest
      );
  ...
  async buildCancelRequest(logisticsMessageId, initMessageId) {
    try {
      ...
      let statusResponse = await productService.productCancel(
        logisticsResponse
      );
  ...
  async buildSellerCancelRequest(
    cancelData,
    logisticsMessageId,
    initMessageId
  ) {
    try {
      ...
      let statusResponse = await productService.productSellerCancel(
        cancelData,
        logisticsResponse
      );
  ...
  async buildSupportRequest(logisticsMessageId, initMessageId) {
    try {
      ...
      let selectResponse = await productService.productSupport(
        logisticsResponse
      );
