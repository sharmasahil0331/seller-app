...
class ProductService {
  ...
  async search(requestQuery) {
    try {
      logger.log(
        "info",
        `[Product Service] search product : param :`,
        requestQuery
      );

      //get search criteria
      const searchProduct =
        requestQuery.message.intent.item?.descriptor?.name ?? "";
      const searchCategory =
        requestQuery.message.intent.category?.descriptor?.id ?? "";
      ...
      logger.log(
        "info",
        `[Product Service] search product : result :`,
        result.data
      );
      ...
      logger.log(
        "info",
        `[Product Service] search product transformed: result :`,
        productData
      );
  ...
  async select(requestQuery) {
    ...
    logger.log(
      "info",
      `[Product Service] checking if logistics provider available from :`,
      logisticData
    );

    let logisticProvider = {};
    for (let logisticData1 of logisticData) {
      //check if any logistics available who is serviceable
    ...
    if (Object.keys(logisticProvider).length === 0) {
      return {
        context: { ...selectData.context, action: "on_select" },
        message: {
          type: "CORE-ERROR",
          code: "60001",
    ...
    logger.log(
      "info",
      `[Product Service] logistics provider available  :`,
      logisticProvider
    );
    ...
    let deliveryCharges = {
      ...
      price: {
        currency:
          "" +
          logisticProvider?.message?.catalog["bpp/providers"][0].items[0].price
            .currency,
        value:
          "" +
          logisticProvider?.message?.catalog["bpp/providers"][0].items[0].price
    ...
    let fulfillments = [
      {
        ...
        "@ondc/org/provider_name":
          logisticProvider?.message?.catalog["bpp/descriptor"],
        tracking: false,
        "@ondc/org/category":
          logisticProvider?.message?.catalog["bpp/providers"][0].category_id,
        "@ondc/org/TAT": "PT45M",
        provider_id: logisticProvider.context.bpp_id,
        state: {
          descriptor: {
            name: logisticProvider?.message?.catalog["bpp/providers"][0]
              ?.descriptor?.name,
          },
        },
  ...
  async productStatus(requestQuery, statusRequest = {}, unsoliciated, payload) {
    ...
    if (
      logisticData.message.order.fulfillments[0].state?.descriptor?.code ===
      "Pending"
    ) {
    ...
    updateOrder.fulfillments[0].state =
      logisticData.message.order.fulfillments[0].state;
  ...
  async productUpdate(requestQuery) {
    ...
    for (let item of updateOrder.items) {
      let updateItem = statusRequest.message.order.items.find((itemObj) => {
  ...
  async productCancel(requestQuery) {
    ...
    updateOrder.state = logisticData.message.order.state;
    updateOrder.cancellation_reason_id =
      cancelRequest.message.cancellation_reason_id;
  ...
  async productSellerCancel(cancelData, requestQuery) {
    ...
    updateOrder.state = logisticData.message.order.state;
    updateOrder.cancellation_reason_id =
      cancelData.message.order.cancellation_reason_id;
  ...
  async productConfirm(requestQuery) {
    ...
    let confirmRequest = JSON.parse(
      JSON.stringify(requestQuery.retail_confirm[0])
    ); //select first select request
    ...
    confirmRequest.message.order.fulfillments[0]["@ondc/org/provider_name"] =
      "LoadShare Delivery"; //TODO: hard coded
    confirmRequest.message.order.payment[
      "@ondc/org/buyer_app_finder_fee_type"
    ] = "percentage"; //TODO: hard coded
  ...
  async productInit(requestQuery) {
    ...
    console.log("🚀 ~ file: product.service.js:1050 ~ ProductService ~ productInit ~ initData:", initData)
    ...
    totalPrice =
      parseInt(logisticData.message.order.quote.price.value) +
      parseInt(totalPrice);
    let totalPriceObj = { value: "" + totalPrice, currency: "INR" };
  ...
  async productSelect(requestQuery) {
    try {
      ...
      console.log(
        "🚀 ~ file: product.service.js:1159 ~ ProductService ~ productSelect ~ requestQuery:",
        JSON.stringify(requestQuery.retail_select[0]),
        requestQuery
      );
      ...
      console.log(
        "🚀 ~ file: product.service.js:1169 ~ ProductService ~ productSelect ~ selectData:",
        selectData
      );
      ...
      for (let logisticData1 of logisticData) {
        if (logisticData1.message) {
          if (
            logisticData1.context.bpp_id ===
            "dtdc-ondc-integration-preprod-demo.demoprojectxsandbox3.shipsy.io"
          ) {
            //TODO: move to env
            if (logisticData1.message) {
              logisticProvider = logisticData1;
      ...
      for (let item of items) {
        ...
        if (isServiceable) {
          console.log(
            "🚀 ~ file: product.service.js:1268 ~ ProductService ~ productSelect ~ isServiceable:",
            isServiceable
          );
          itemObj.fulfillment_id =
            logisticProvider.message.catalog[
              "bpp/providers"
            ][0].items[0].fulfillment_id; //TODO: revisit for item level status

          deliveryType = logisticProvider.message.catalog[
            "bpp/providers"
      ...
      console.log(
        "🚀 ~ file: product.service.js:1409 ~ ProductService ~ productSelect ~ isServiceable:",
        isServiceable,
        "deliveryType",
        deliveryType
      );
      if (isServiceable && deliveryType) {
        ...
        let categories =
          logisticProvider.message.catalog["bpp/providers"][0].categories;
        let duration = "";
        if (deliveryType?.time?.duration) {
          ...
          console.log(
            "🚀 ~ file: product.service.js:1330 ~ ProductService ~ category ~ category:",
            category
          );
          duration = "P2D";
        }
        ...
        fulfillments = [
          {
            ...
            "@ondc/org/provider_name":
              logisticProvider.message.catalog["bpp/descriptor"].name,
        ...
        const org = await this.getOrgForOndc(
          selectData.message.order.provider.id
        );
        deliveryCharges = {
          title: "Delivery charges",
          "@ondc/org/title_type": "delivery",
        ...
        fulfillments = [
          {
            ...
            "@ondc/org/category":
              config.get("sellerConfig").LOGISTICS_DELIVERY_TYPE,
            "@ondc/org/TAT": "P1D",
      ...
      savedLogistics.logisticsTransactionId =
        logisticProvider?.context?.transaction_id;
